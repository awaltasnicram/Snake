...
def main():
    ...
    c_draw = tk.Canvas(okno, width = 600, height = 400) # tworzenia canvasa
    c_draw.pack() # paczka canvas
    ...
    class Snake: # klasa snake
        ...
        def eat(self): # waz musi zjesc
            if self.msnake[0][0] == self.food[0] and self.msnake[0][1] == self.food[1]: # warunek, gdy waz jest na jedzeniu to je
                self.msnake.append([0,0]) # element powiekszajacy weza
                self.food = [rn.randint(1, self.width - 2), rn.randint(1, self.height - 2)] # nowe losowe jedzenie dla weza
        def move_snake(self):
    ...
    menu.add_command(label="Wyjście", command = okno.quit)
